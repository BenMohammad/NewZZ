...
import android.provider.VoicemailContract.Status.ITEM_TYPE
...
import com.benmohammad.newzz.ui.ITEM_TYPE.SMALL
import com.benmohammad.newzz.ui.NewsItemAdapter
import com.benmohammad.newzz.ui.home.banner.BannerAdapter
import com.benmohammad.newzz.ui.home.banner.ZoomOutPageTransformer
...
class HomeFragment: Fragment(), CoroutineScope {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        retrySnack = Snackbar.make(dashContainer, "You are offline!!", Snackbar.LENGTH_SHORT).setAction("Retry") {
            refresh()
        }
    ...
    private suspend fun setCategoryItems(type: String, isRefresh: Boolean) {
        try {
            val items = dashViewModel.getItemAsync(type, isRefresh).await()
            if (items.isNotEmpty()) {
                when (type) {
                    "New" -> {
    ...
    private suspend fun setBannerItems(isRefresh: Boolean = false) {
        try {
            val topItems = dashViewModel.getItemAsync("Top", isRefresh).await()
            shimmerTop.apply{
                stopShimmer()
                isVisible = false
            }
